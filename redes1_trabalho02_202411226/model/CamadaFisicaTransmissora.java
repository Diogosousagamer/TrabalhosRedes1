...
* Funcao...........: Esta classe tem como objetivo gerenciar os eventos e operacoes da camada fisica da maquina
                     transmissora, que codificara os dados antes de serem transmitidos atraves do meio de transmissao.
...
public class CamadaFisicaTransmissora {
  ...
  // Vetor que armazenara o fluxo bruto de bits
  ...
  /*
   * ***************************************************************
   * Metodo: executarCamadaFisicaTransmissora
   * Funcao: executa as operacoes da camada fisica transmissora
   * Parametros: int[] quadro - vetor de bits a serem codificados
   * Retorno: void
   ****************************************************************/

  protected void executarCamadaFisicaTransmissora(int[] quadro) {
    // Obtem-se o tipo de codificacao selecionado pelo usuario
    int tipoCodificacao = TelaPrincipalController.controller.getTipoCodificacao();

    // Inicio do bloco switch/case
    // A codificacao sera realizada de acordo com o metodo selecionado pelo usuario
    ...
    // Inicializa o meio de transmissao para transmitir o fluxo de bits a maquina receptora
    MeioDeTransmissao transmissao = new MeioDeTransmissao();
    transmissao.MeioTransmissao(fluxoBrutoDeBits);
  }
  ...
  private int[] CamadaFisicaTransmissoraCodificacaoBinaria(int[] quadro) {
    // Vetor que armazenara o fluxo de bits gerado
    ...
    // Inicio do bloco for
    // Percorre todo o quadro para que a codificacao seja realizada
    for (int i = 0; i < quadro.length; i++) {
      // Inicio do bloco for
      // Computa todos os 32 bits que compoem o inteiro
      for (int j = 31; j >= 0; j--) {
        // Um bit e extraido do quadro
        int bit = (quadro[i] >> j) & 1;

        // O bit e inserido dentro do vetor
        resultado[posicao] |= bit;
   ...
   * Funcao: codifica os bits atraves da codificacao manchester diferencial
   * Parametros: int[] quadro - vetor de bits a serem codificados
   * Retorno: int[]
   ****************************************************************/
